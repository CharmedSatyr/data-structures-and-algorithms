...
const isCapitalized = str => str.split(' ').filter(word => /^[A-Z]\w*/g.test(word));
...
const citiesAtoJ = arr => arr.filter(c => /^[A-J]\w*/g.test(c));
...
const matchMonth = input => /^[Oo]ct(ober)?\b/.test(input);
...
const findShells = str => str.match(/\bs\w{0,5}ells/g);
