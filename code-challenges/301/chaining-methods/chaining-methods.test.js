...
const count = (target, input) => {
  ...
  return flat.filter(num => num === target).length;
};
...
const divisibleByFiveTwoToThePower = input => {
  ...
  return filtered.map(arr => arr.map(num => 2 ** num));
};
...
let findMaleAndFemale = data => {
  const filtered = data.filter(c => c.gender === 'male' || c.gender === 'female');
...
let findShortest = data => {
  const heights = data.map(h => parseInt(h.height));
...
describe('Testing challenge 3', () => {
  ...
  test('It should return an empty array if none of the numbers are divisible by five', () => {
    expect(divisibleByFiveTwoToThePower([[1, 2, 3], [5, 10, 15]])).toStrictEqual([
      [],
      [32, 1024, 32768],
    ]);
  });
