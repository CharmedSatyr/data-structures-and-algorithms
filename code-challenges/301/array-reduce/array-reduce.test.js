...
const countNumberOfElements = arr => arr.reduce((acc, curr) => (acc += 1), 0);
...
const returnNames = arr => arr.reduce((acc, curr) => acc.concat(curr.name), []);
...
const countNumberOfChildren = arr =>
  arr.reduce((acc, curr) => (curr.children ? acc + curr.children.length : acc), 0);
...
const countPrimeNumbers = arr => arr.reduce((acc, curr) => (isPrime(curr) ? (acc += 1) : acc), 0);
...
const extractStat = (statName, arr) =>
  arr.reduce((acc, curr) => (curr.stat.name === statName ? curr : acc), null);
...
const extractChildren = arr =>
  arr
    .filter(char => char.name.indexOf('a') !== -1)
