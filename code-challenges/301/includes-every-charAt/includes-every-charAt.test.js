...
const standardizePhoneNumbers = arr => arr.map(str => str.replace(/[\(\)\-\s]/g, ''));
...
const unenrollBrook = arr => arr.map(ar => ar.filter(str => !str.includes('Brook')));
...
const sortByDay = arr => daysOfWeek.map(day => arr.filter(ar => ar.includes(day)));
