...
const validateEmail = email =>
  /\b([a-z0-9]+\.)?([a-z0-9])+@[a-z0-9]+\.(net|com|org)\b/gi.test(email);
...
const validatePhoneNumber = phoneNumber =>
  /\(?\d{3}\)?(\-|\s)?\d{3}(\-|\s)?\d{4}/g.test(phoneNumber);
...
const findTagNames = elements => {
  const result = [];
  const regex = /\/\w+/g;
  elements.forEach(e => result.push(...e.match(regex)));
  return result;
};
...
describe('Testing challenge 4', () => {
  test('It should return the closing tags', () => {
    expect(findTagNames(['<h1>Hello, world!</h1>', '<p>Welcome to my site</p>'])).toStrictEqual([
      '/h1',
      '/p',
    ]);
  });
