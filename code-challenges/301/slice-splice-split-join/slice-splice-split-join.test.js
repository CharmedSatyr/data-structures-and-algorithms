...
const listFoods = recipe => {
  ...
  const units = ['cup', 'gallon', 'medium-sized', 'pound'];
  ingredients.forEach(ingredient => {
    const afterFirstSpace = ingredient.indexOf(' ') + 1;
    const numberlessIngredient = ingredient.slice(afterFirstSpace);
    units.forEach(unit => {
      const unitIndex = numberlessIngredient.indexOf(unit);
      if (unitIndex === 0) {
        const name = numberlessIngredient.slice(unit.length + 1).trim();
        list.push(name);
      }
    });
  });
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  const end = str.length - numberOfCharacters;
...
const totalSumCSV = str => str.split(',').reduce((acc, curr) => acc + parseInt(curr), 0);
