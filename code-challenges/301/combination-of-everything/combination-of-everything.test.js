...
let biggerThanLuke = arr => {
  const luke = arr.find(o => o.name === 'Luke Skywalker');
  if (luke) {
    const lukeMass = luke.mass;

    return arr
      .filter(o => parseInt(o.mass) > lukeMass)
      .map(o => o.name)
      .join(' - ');
  }
  return '';
...
const sortBy = (property, arr) =>
  arr.sort((a, b) => {
    if (typeof a[property] === 'string' && typeof b[property] === 'string') {
      const al = a[property].toLowerCase();
      const bl = b[property].toLowerCase();
      if (al === bl) {
        return 0;
      }
      return al > bl ? 1 : -1;
...
------------------------------------------------------------------------------------------------ */
const isSecure = url => /^https:\/{2}.*/g.test(url);
...
const detectTicTacToeWin = board => {
  const helpCheck = (player, b) => {
    ...
    return allwins.some(indices => {
      return b[indices[0]] == player && b[indices[1]] == player && b[indices[2]] == player;
    });
  };
  const flat = [].concat(...board);
  if (helpCheck('X', flat) || helpCheck('O', flat)) {
    return true;
  }
  return false;
};
...
describe('Testing challenge 5', () => {
  test('It should return true if there are three in a row', () => {
    expect(detectTicTacToeWin([['X', '', 'O'], ['X', 'O', ''], ['X', 'O', 'X']])).toStrictEqual(
      true
    );
    expect(detectTicTacToeWin([['O', '', 'X'], ['X', 'O', 'X'], ['X', '', 'O']])).toStrictEqual(
      true
    );
  });

  test('It should return false if there are not three in a row', () => {
    expect(detectTicTacToeWin([['X', '', 'O'], ['O', 'O', ''], ['X', 'O', 'X']])).toStrictEqual(
      false
    );
  });
