...
const filterStringsWithVowels = arr => arr.filter(str => /[aeiou]/g.test(str));
...
const notInFirstArray = (forbiddenValues, arr) =>
  arr.filter(val => forbiddenValues.indexOf(val) === -1);
...
const getStatName = (arr, minBaseStat) =>
  arr.filter(stat => stat.baseStat > minBaseStat).map(s => s.stat.name);
...
const getCharactersWithoutChildren = arr => arr.filter(c => !c.children || c.children.length === 0);
...
const evenOddNumericValues = arr =>
  arr.filter(val => typeof val === 'number').map(num => (num % 2 === 0 ? 'even' : 'odd'));
