...
const alphabetizeBetter = arr =>
  arr.sort((a, b) => {
    ...
    const comparison = al.localeCompare(bl);
    if (comparison === 0) {
...
const sortPeopleBetter = arr =>
  arr.sort((a, b) => {
    ...
    if (comparison === 0) {
      ...
      const compareF = af.localeCompare(bf);
      if (compareF === 0) {
        return a.age - b.age;
      }
...
const sortSchedule = arr => {
  const byTime = arr.sort((a, b) => {
    if (a.start === b.start) {
      const aLen = parseInt(a.end) - parseInt(a.start);
      const bLen = parseInt(b.end) - parseInt(b.start);
      return aLen - bLen;
    }
    return a.start - b.start;
  });
  return sortMeetingsByDay(byTime);
};
